...
Log 图标重绘为完整一体图形：文档外框（日志载体）+ 上半两条日志行 + 下半下载
箭头，六笔同框；图标本体放大到 22px 增强辨识。

## 1.4.23

Log 图标加双层投影（drop-shadow 贴合遮罩形状）：近距 1px/30% + 远距 2px/18%，
裸图标有了浮起感。

## 1.4.22

Log 图标彻底去按钮化：边框/底色/悬停底全透明，仅留 32px 隐形触区 + 20px 图标本体。

## 1.4.21

Log 下载胶囊改**纯图标按钮**：32×32 圆角方，自绘 SVG（折角文档 + 下行箭头，
2px 圆头描边）经 CSS mask 绘制于 ::after，React 重渲染零冲突；文字由重写器清空
并补 title 提示。

## 1.4.20

设置抽屉：去掉 tabs 与内容之间的空白带——内容区分区标题头（54px 高、20px 上内距，
仅含与 tabs 重复的分区名和已隐藏的 X）整个隐藏，内容区顶部改留 6px 呼吸位。

## 1.4.19

设置弹窗改**底部抽屉**：面板贴屏底、上滑入场动画、顶部留 48px 遮罩带、仅上圆角、
底部安全区内边距；头部 X 关闭按钮隐藏（手机层），点击遮罩关闭（JS 经仍在
DOM 的关闭按钮触发）；头部高度收紧。

## 1.4.18

设置弹窗·通用页：隐藏"打开配置文件"操作行（`.me01iq_action`，错误提示 + outline
按钮）——手机端编辑配置文件不实用，同时消掉其上方过大间距。
