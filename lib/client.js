...
@media (max-width:820px){
  ...
  *::-webkit-scrollbar{display:none!important;width:0!important;height:0!important}
  ...
  /* (1.4.8) per user: no 2-row split — just DROP the breadcrumb title on
     phones (it fought the FAB clearance for width); utilities + actions
     keep the single row, right-aligned. Tabs row below is untouched. */
  .wSkVaW_titleCluster{display:none!important}
...
`;
		...
		// tapping a tool call's inspect affordance opens the details drawer
			...
			const selfCheck = setTimeout(() => {
				...
				window.__dshMobileUi = { version: "1.4.8", frameTagged: !!(frameEl && frameEl.querySelector("[data-dshm-col]")), missing };
