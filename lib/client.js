...
@media (max-width:820px){
  ...
  *::-webkit-scrollbar{display:none!important;width:0!important;height:0!important}
  ...
  /* (1.4.12) while better-sidebar's right cluster is out (collapsed state),
     the header yields the corner: cluster spans right 8→96px (2×40px + gaps),
     so right-pad 104px — mirrors better-sidebar's own desktop 78px trick,
     resized for our 40px buttons */
  body[data-dsh-sidebar-collapsed] .wSkVaW_header{padding-right:calc(env(safe-area-inset-right,0px) + 104px)!important}
  /* (1.4.9) drop ONLY the breadcrumb path (the "title") on phones. The
     ...
     actions keep their right-side spot beside utilities. Tabs untouched. */
  .wSkVaW_crumbs{display:none!important}
  .wSkVaW_tabs{gap:20px!important}
  .wSkVaW_headerActions{gap:4px!important;margin-left:auto!important}
  ...
  .nL4_yW_sessionLogButton{min-width:0!important}
  /* (1.4.13) the three mid-header chips ship as mismatched widgets —
     preset pill .SVAs4q_label (22px/6px tint chip), subagent trigger
     .h8S2Va_trigger (28px/6px flat), log capsule .nL4_yW_sessionLogButton
     (32px/18px bordered). One uniform capsule row on phones */
  .SVAs4q_label,.h8S2Va_trigger,.nL4_yW_sessionLogButton{
    height:32px!important;min-height:32px!important;border-radius:999px!important;
    align-items:center!important;padding:0 12px!important;gap:4px!important;
    font-size:12px!important;line-height:16px!important;white-space:nowrap!important;
  ...
  .SVAs4q_label{max-width:32vw!important}
  .wSkVaW_headerActions,.wSkVaW_headerUtilities{gap:6px!important;align-items:center!important}
...
`;
		...
		// tapping a tool call's inspect affordance opens the details drawer
			...
			const selfCheck = setTimeout(() => {
				...
				window.__dshMobileUi = { version: "1.4.13", frameTagged: !!(frameEl && frameEl.querySelector("[data-dshm-col]")), missing };
