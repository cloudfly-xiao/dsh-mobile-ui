...
@media (max-width:640px){
  /* (1.4.3) dialog breathes: 12px inset from the screen edges instead of the
     ...
     so every layer inside sits off the border; the tabs strip keeps its
     full-bleed divider via -10px horizontal margins, and the options column
  ...
  .VOzbGW_nav{width:100%!important;flex:none!important;flex-direction:row!important;align-items:center;gap:4px!important;margin:0 -10px 6px!important;padding:6px 10px 8px!important;overflow-x:auto;overflow-y:hidden;border-bottom:1px solid var(--dsw-alias-border-l2)}
...
`;
		...
		// tapping a tool call's inspect affordance opens the details drawer
			...
			const selfCheck = setTimeout(() => {
				...
				window.__dshMobileUi = { version: "1.4.4", frameTagged: !!(frameEl && frameEl.querySelector("[data-dshm-col]")), missing };
