...
@media (max-width:640px){
  ...
  /* (1.4.6) nav tabs wrap into a 2-row grid instead of one squeezed strip:
     cells get a 40% flex-basis (two per row, evenly stretched, labels
     centered) and the strip stops force-scrolling horizontally */
  .VOzbGW_nav{width:100%!important;flex:none!important;flex-direction:row!important;align-items:center;gap:4px!important;padding:6px 10px 8px!important;border-bottom:1px solid var(--dsw-alias-border-l2)}
  .VOzbGW_navTitle{display:none!important}
  .VOzbGW_navList{flex:1 1 100%!important;flex-direction:row!important;flex-wrap:wrap!important;gap:4px!important}
  .VOzbGW_navCell{flex:1 1 40%!important;justify-content:center!important;white-space:nowrap!important;height:34px!important;padding:0 8px!important}
...
`;
		...
		// tapping a tool call's inspect affordance opens the details drawer
			...
			const selfCheck = setTimeout(() => {
				...
				window.__dshMobileUi = { version: "1.4.6", frameTagged: !!(frameEl && frameEl.querySelector("[data-dshm-col]")), missing };
