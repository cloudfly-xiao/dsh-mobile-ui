...
@media (max-width:820px){
  ...
  *::-webkit-scrollbar{display:none!important;width:0!important;height:0!important}
  ...
  /* (1.4.21→25) Log capsule → plain "log↓" TEXT mark, per user: no button
     chrome, no custom glyph — the relabel pass writes the literal label,
     the shipped icon stays hidden, a soft text shadow keeps the lift */
  .nL4_yW_sessionLogButton{
    min-width:0!important;padding:0 2px!important;gap:0!important;
  ...
  /* (1.4.26) one text tone across the header trio — preset pill ships
     label-secondary, subagent trigger label-tertiary, log button
     label-primary; unify all three on label-secondary */
  .SVAs4q_label,.h8S2Va_trigger,.nL4_yW_sessionLogButton{color:var(--dsw-alias-label-secondary)!important}
...
`;
		...
		function apply(ctx) {
			...
			const relabelCapsule = () => {
				...
				for (let n = walker.nextNode(); n; n = walker.nextNode())
					...
					n.nodeValue = "log\u2193"; // (1.4.25) plain text mark per user
		...
		// tapping a tool call's inspect affordance opens the details drawer
			...
			const selfCheck = setTimeout(() => {
				...
				window.__dshMobileUi = { version: "1.4.26", frameTagged: !!(frameEl && frameEl.querySelector("[data-dshm-col]")), missing };
