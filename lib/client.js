...
@media (max-width:820px){
  ...
  body[data-mob-sidebar-open='1'] #root div[style*='grid-template-columns']>[data-dshm-col='sidebar']{transform:translateX(0)}
  ...
  body[data-mob-panel='details'] #root div[style*='grid-template-columns']>[data-dshm-col='details']{transform:translateX(0)}
  ...
  /* --- dshmarket plugin market (v1.11.x css-scope eGUBIq_, phone layer) ---
     ...
     diagnostics meta wraps instead of ellipsizing at 320px. The install
     dialogs use the host Modal primitive (shell css-scope _15u5s_): it goes
     near-full-bleed with a viewport-capped height (vh fallback, dvh upgrade). */
  .eGUBIq_titleRow{flex-wrap:wrap!important;gap:6px 8px!important}
  .eGUBIq_sub{flex-wrap:wrap!important;gap:6px 8px!important}
  .eGUBIq_tabs{overflow-x:auto!important;scrollbar-width:none;gap:0!important}
  .eGUBIq_tabs::-webkit-scrollbar{display:none}
  ...
  ._root_15u5s_2{padding:12px!important}
  ._dialog_15u5s_22{width:100%!important;max-height:calc(100vh - 24px)!important;max-height:calc(100dvh - 24px)!important;overflow-y:auto!important}
  ._body_15u5s_89{margin-top:12px!important}
  ._footer_15u5s_97{flex-wrap:wrap!important}
}
...
`;
				...
				for (const m of muts) {
					...
					for (const list of [m.addedNodes, m.removedNodes]) {
						for (const n of list) {
							...
							if ((n.matches && n.matches(SLOT_QUERY)) || (n.querySelector && n.querySelector(SLOT_QUERY))) { queue(true, false); return; }
						}
					}
				}
			};
		...
		// tapping a tool call's inspect affordance opens the details drawer
			...
			const selfCheck = setTimeout(() => {
				...
				window.__dshMobileUi = { version: "1.3.0", frameTagged: !!(frameEl && frameEl.querySelector("[data-dshm-col]")), missing };
