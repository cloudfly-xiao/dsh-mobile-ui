...
@media (max-width:820px){
  ...
  *::-webkit-scrollbar{display:none!important;width:0!important;height:0!important}
  ...
  /* (1.4.15→16) corner reservations, UNCONDITIONAL and now MEASURED: the
     better-sidebar cluster renders ONE 40px button at right:8 (verified via
     CDP: x=vw-48), so both sides reserve exactly 8+40+6=54px — header content
     spans [54, vw-54] and the first/last chips sit 6px off each corner icon.
     (1.4.12's 104px assumed two cluster buttons; the gate on
     body[data-dsh-sidebar-collapsed] never fired either — attribute absent) */
  .wSkVaW_header{padding:calc(env(safe-area-inset-top,0px) + 8px) calc(env(safe-area-inset-right,0px) + 54px) 0 54px!important}
  /* (1.4.9) drop ONLY the breadcrumb path (the "title") on phones. The
     ...
     (1.4.14) chips spread evenly between the two corner icons: the two slot
     wrappers and their cluster flatten via display:contents, so the header
     chips become direct flex items of the title row, and space-between
     distributes them across the corner-reserved padding. Chip skins are the
     shipped ones (the 1.4.13 restyle was reverted on purpose). */
  .wSkVaW_crumbs{display:none!important}
  .wSkVaW_titleRow{justify-content:space-between!important}
...
`;
		...
		// tapping a tool call's inspect affordance opens the details drawer
			...
			const selfCheck = setTimeout(() => {
				...
				window.__dshMobileUi = { version: "1.4.16", frameTagged: !!(frameEl && frameEl.querySelector("[data-dshm-col]")), missing };
